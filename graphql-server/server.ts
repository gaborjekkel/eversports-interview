...
const resolvers = {
  Query: {
    products: async (_: any, { searchTerm, first, last, before, after }: PaginatedArgs) => {
    ...
    users: async (_: any, { searchTerm, first, last, before, after }: PaginatedArgs) => {
      // Validate pagination arguments
      if (first && last) {
        throw new Error("Cannot use 'first' and 'last' together.");
      }
    ...
    purchases: async (_: any, { productIds, userIds, first, last, before, after }: PurchasesArgs) => {
      await delay(randomDelay());
  ...
  Purchase: {
    user: (purchase: Purchase) => users.find(user => user.id === purchase.userId),
    product: (purchase: Purchase) => products.find(product => product.id === purchase.productId),
  },
};
