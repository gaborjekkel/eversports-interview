...
import { ErrorMessage, LoadingMessage, NoSearchFoundMessage } from "./StateMessages";
...
const RenderList = function({ list }: RenderListProps) {
  ...
  return (
    <React.Fragment>
          ...
          <div className="flex px-[15px] py-[10px] justify-between">
            ...
            <Button text={'Apply'} onClick={() => console.log('click')}/>
          </div>
        </React.Fragment>
  )
}
...
export default function CheckableList(props: CheckableListProps) {
  ...
  let Renderer = <ErrorMessage />

  if(error) {
    Renderer
  } else if(loading) {
    Renderer = <LoadingMessage />;
  } else if(!error && !loading && data.products.nodes) {
    Renderer = <RenderList list={data.products.nodes}/>;
  } else {
    Renderer
  }
